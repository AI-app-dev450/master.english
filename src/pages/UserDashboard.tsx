...
export function UserDashboard() {
  ...
  const handleCancelEdit = () => {
    ...
    setCefrLevel((currentUser?.cefrLevel as CEFRLevel) || 'A2');
    setDailyGoal(currentUser?.dailyGoal || 10);
    setEditing(false);
  };

  const handleChangePassword = () => {
    if (!newPw || !currentPw) { addToast('Please fill all password fields', 'error'); return; }
        ...
        <div className="px-6 pt-6 pb-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              {/* Avatar */}
              <div className={`h-16 w-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white flex-shrink-0 ${
                isAdmin ? 'bg-[#F5A623]' : 'bg-[#4A90E2]'
              }`}>
              ...
              <div>
                ...
                <p className="text-xs text-white/40 mt-0.5">
                  {currentUser?.cefrLevel} level · Joined {new Date(currentUser?.joinDate || '').toLocaleDateString()}
                </p>
              </div>
            </div>
            {/* Logout Button */}
            <button
              ...
              className="flex items-center gap-1.5 text-white/50 hover:text-white text-sm transition-colors flex-shrink-0 border border-white/10 hover:border-white/30 px-3 py-1.5 rounded-lg"
            >
          ...
          <div className="mt-5">
            <div className="flex items-center justify-between text-xs text-white/50 mb-1.5">
              <span className="flex items-center gap-1"><Flame className="h-3 w-3 text-[#F5A623]" /> Today's Progress</span>
          ...
          <div className="grid grid-cols-3 gap-3 mt-4">
            {[
              { label: 'Streak', value: stats.currentStreak, suffix: ' days', icon: Flame },
              { label: 'Mastered', value: stats.learnedWords, suffix: ' words', icon: CheckCircle2 },
              { label: 'Total', value: stats.totalWords, suffix: ' words', icon: BookOpen },
            ].map(item => (
              <div key={item.label} className="text-center bg-white/5 rounded-xl py-3">
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                { label: 'Mastery Rate', value: `${progressPercent}%`, icon: TrendingUp, color: 'text-purple-600', bg: 'bg-purple-50' },
            ...
            {vocabulary.sessions.length > 0 && (
              <div className="bg-card rounded-2xl border border-border p-5">
                <h3 className="text-sm font-semibold text-foreground flex items-center gap-2 mb-4">
                  <GraduationCap className="h-4 w-4 text-[#4A90E2]" strokeWidth={1.5} />
                  ...
                  {vocabulary.sessions.slice(0, 5).map(s => (
                    <div key={s.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-muted-foreground">{s.wordsStudied} words</span>
            ...
            <div className="bg-card rounded-2xl border border-border p-5">
              <h3 className="text-sm font-semibold text-foreground flex items-center gap-2 mb-4">
                <Award className="h-4 w-4 text-[#F5A623]" strokeWidth={1.5} />
                Achievements
              </h3>
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-5">
              <div className="flex items-center justify-between">
                <h2 className="font-semibold text-foreground flex items-center gap-2">
                  <User className="h-4 w-4 text-[#4A90E2]" strokeWidth={1.5} />
                        ...
                        <button
                          ...
                          className={`px-3.5 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                            cefrLevel === l
                              ...
                              : 'bg-card text-muted-foreground border-border hover:border-[#F5A623]/50'
                          }`}
                        >
                          {l}
                        </button>
        ...
        {activeTab === 'security' && (
          ...
          >
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-4">
              <h2 className="font-semibold text-foreground flex items-center gap-2">
                <Lock className="h-4 w-4 text-[#4A90E2]" strokeWidth={1.5} />
                Change Password
              </h2>
                ...
                <div key={field.label}>
                  ...
                  <div className="relative">
                    <input
                      ...
                      placeholder={field.label === 'New Password' ? 'Min 6 characters' : '••••••••'}
            ...
            <div className="bg-card rounded-2xl border border-border p-5 space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Sign out of your account on this device. Your progress and data will be saved.
              </p>
              <button
                onClick={() => setShowLogoutConfirm(true)}
